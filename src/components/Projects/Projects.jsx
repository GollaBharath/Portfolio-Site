...
class ArcballControl {
	...
	navigateWithKeyboard(direction) {
		const ROTATION_ANGLE = 0.15; // radians per key press
		const axis = vec3.create();

		switch (direction) {
			case "ArrowUp":
				vec3.set(axis, 1, 0, 0); // Rotate around X axis
				break;
			case "ArrowDown":
				vec3.set(axis, -1, 0, 0); // Rotate around X axis (opposite)
				break;
			case "ArrowLeft":
				vec3.set(axis, 0, 1, 0); // Rotate around Y axis
				break;
			case "ArrowRight":
				vec3.set(axis, 0, -1, 0); // Rotate around Y axis (opposite)
				break;
			default:
				return;
		}
		...
		quat.setAxisAngle(rotation, axis, ROTATION_ANGLE);

		// Apply rotation to current orientation
		quat.multiply(this.pointerRotation, rotation, this.pointerRotation);

		// Trigger movement
		this.updateCallback(16); // Simulate a frame update
	}
